...
{
    QVariant translated = val;

    if(val.userType() == filePathTypeId())
    {
        FilePathPropertyType path = qVariantValue<FilePathPropertyType>(val);
        translated.setValue(path.value());
    }
    if(val.userType() == newFileTypeId())
    {
        NewFilePropertyType path = qVariantValue<NewFilePropertyType>(val);
        translated.setValue(path.value());
    }
    if (theValues.contains(property)) {
        if (translated.type() != QVariant::String && !translated.canConvert(QVariant::String))
            return;
        QString str = qVariantValue<QString>(translated);
        ...
        {
            FilePathPropertyType fp(str);
            v.setValue(fp);
        }
        if(propertyType(property) == newFileTypeId())
        {
            NewFilePropertyType fp(str);
            v.setValue(fp);
        }
