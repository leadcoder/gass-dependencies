...
namespace Hydrax
{
	...
	void RttManager::setCamera(Ogre::Camera* camera)
	{
		for(int i = 0 ; i < 6 ;i++)
		{
			Ogre::TexturePtr &tex = mTextures[i];
			if (!tex.isNull())
			{
				Ogre::RenderTarget* RT_Texture = tex->getBuffer()->getRenderTarget();
				RT_Texture->getViewport(0)->setCamera(camera);
			}
		}
