...
void PHPDirectoryServer::Update(void)
{
		...
		(fields.Size()>0 && time > nextUpdate))
    {
		...
		if (timeoutParam==0)
		{
			...
			outgoing += "\002true\001";
		}
		else
		{
			for (unsigned i = 0; i < fields.Size(); i++)
			{
				outgoing += RakString("%s\002%s\001",
		...
		outgoing += "\002";
		outgoing += passwordParam;
		outgoing += "\001";

		http.Post(RakString("%s?update", path.C_String()), outgoing);
    ...
    else if(downloadTableRequested)
    {
		...
		outgoing += passwordParam;
		outgoing += "\001";
        http.Post(RakString("%s?query", path.C_String()), outgoing);
        waitingForReply = true;
