...
unsigned int GrassLayer::_populateGrassList_Uniform(PageInfo page, float *posBuff, unsigned int grassCount)
{
	...
	//parent->rTable->resetRandomIndex(); //JH
	...
	if (shaderNeedsUpdate){
		...
		if (caps->hasCapability(RSC_VERTEX_PROGRAM) && geom->getShadersEnabled())
		{
			...
			if (tmpMat.isNull())
			{
				...
				params->setNamedAutoConstant("worldMatrix", GpuProgramParameters::ACT_WORLD_MATRIX); // ADDED BY JH
				params->setNamedAutoConstant("texViewProj", GpuProgramParameters::ACT_TEXTURE_VIEWPROJ_MATRIX,0); // ADDED BY JH
