...
bool StaticBillboardSet::mDynamicLighting		   =false;
      ...
      if (s_nSelfInstances == 0)
      {
         ...
         if (!s_isGLSL) // DirectX HLSL or nVidia CG
         {
            vertexProg =
               ...
               //Color
               "	oColor = color*ambient + color*diffuse*0.5;   \n"
			   //UV Scroll
               "	oUv = uv;	\n"
               "	oUv.x += uScroll; \n"
               "	oUv.y += vScroll; \n"
         ...
         if (!s_isGLSL) // DirectX HLSL or nVidia CG
         {
            vertexProg2 =
               ...
               "	oColor.rgb = color.rgb*ambient.rgb + diffuse.rgb*color.rgb*0.5;   \n"
   ...
   else
   {
      ...
      for (unsigned short t = 0; t < fadeMaterial->getNumTechniques(); ++t)
      {
         ...
         for (unsigned short p = 0; p < tech->getNumPasses(); ++p)
         {
			...
			//params->setNamedAutoConstant("ambient", GpuProgramParameters::ACT_DERIVED_AMBIENT_LIGHT_COLOUR); //JH

			if(mDynamicLighting)
			{
				params->setNamedAutoConstant("ambient", GpuProgramParameters::ACT_DERIVED_SCENE_COLOUR ); //JH
				params->setNamedAutoConstant("diffuse", GpuProgramParameters::ACT_DERIVED_LIGHT_DIFFUSE_COLOUR); //JH
			}
			else
			{
				Vector4 color;
				color.x = 1;color.y = 1;color.z = 1;color.w = 1;
				params->setNamedConstant("ambient",color);
				params->setNamedConstant("diffuse", color); //JH
			}
            ...
            pass->setAlphaRejectFunction(CMPF_ALWAYS_PASS); //MOD BY JH
            pass->setDepthWriteEnabled(false); //MOD BY JH
...
void StaticBillboardSet::updateAll(const Vector3 &cameraDirection)
{
   ...
   while (i1 != iend)
   {
      ...
      if (!p->hasVertexProgram())
      {
		 ...
		 //p->getVertexProgramParameters()->setNamedAutoConstant("ambient", GpuProgramParameters::ACT_DERIVED_AMBIENT_LIGHT_COLOUR); //JH
		 if(mDynamicLighting)
		 {
			 p->getVertexProgramParameters()->setNamedAutoConstant("ambient", GpuProgramParameters::ACT_DERIVED_SCENE_COLOUR ); //JH
			 p->getVertexProgramParameters()->setNamedAutoConstant("diffuse", GpuProgramParameters::ACT_DERIVED_LIGHT_DIFFUSE_COLOUR); //JH
		 }
		 else
		 {
			 Vector4 color;
			 color.x = 1;color.y = 1;color.z = 1;color.w = 1;
			 p->getVertexProgramParameters()->setNamedConstant("ambient",color);
			 p->getVertexProgramParameters()->setNamedConstant("diffuse", color); //JH
		 }
