...
loader(0)
{
   ...
   ImpostorTexture::sceneMgr = group->getParentPagedGeometry()->getSceneManager();
	...
	for (int i = 0; i < IMPOSTOR_PITCH_ANGLES; ++i){
		...
		//p->setLightingEnabled(true); //JH
		p->setAmbient(1,1,1);//JH
		p->setDiffuse(1,1,1,1);//JH
		p->setSelfIllumination(0,0,0);//JH
		...
		{
			p->setAlphaRejectSettings(CMPF_GREATER_EQUAL, group->getParentPagedGeometry()->getImposterAlphaRejectValue()); //CHANGED BY: JH
...
void ImpostorTexture::renderTextures(bool force)
{
	...
	if (renderTexture.isNull())
   {
	...
	//sceneMgr->setFog(FOG_NONE); // MOD JH
	sceneMgr->setFog(sceneMgr->getFogMode(),sceneMgr->getFogColour(), sceneMgr->getFogDensity(), 1000000, 2000000);
   ...
   Ogre::Real oldMaxDistance = entity->getRenderingDistance();
	...
	renderTarget->update(); //prepare JH
	if (needsRegen){
		...
		//also save dds! JH
		renderTarget->writeContentsToFile(tempdir + fileNameDDS);
