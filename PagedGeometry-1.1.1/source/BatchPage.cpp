		...
		if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull())
		{
			...
			if(!shaderLanguage.compare("hlsl") || !shaderLanguage.compare("cg"))
			{
				...
				if (lightingEnabled)
				{
					...
					vertexProgSource +=
						"	float3 lightDir = objSpaceLight.xyz - (iPosition.xyz * objSpaceLight.w); \n"
						"   float lightDist = length(lightDir); \n"
						"   lightDir = normalize(lightDir); \n"
						"	float diffuseFactor = max(dot(normal, lightDir), 0); \n"
						"	float4 litRes = float4(0,0,0,1); \n";
						//"   if(lightDist < lightAtten.x)\n"
						//"   {\n";
					if (subBatch->m_pVertexData->vertexDeclaration->findElementBySemantic(VES_DIFFUSE) != NULL)
						vertexProgSource += "litRes = lightDiffuse *diffuseFactor* iColor; \n";
					else
						vertexProgSource += "litRes = lightDiffuse * diffuseFactor; \n";
					...
					vertexProgSource += 
						...
						"litRes.xyz = attn*spotFactor*litRes.xyz;\n"
						//"}\n"
						"oColor = litRes + lightAmbient;\n";
				...
				for (unsigned short p = 0, pCnt = tech->getNumPasses(); p < pCnt; ++p)
				{
					...
					try
					{
						...
						if (lightingEnabled)
						{
							...
							//params->setNamedAutoConstant("lightAmbient", GpuProgramParameters::ACT_DERIVED_SCENE_COLOUR);
							

							params->setNamedAutoConstant("lightAtten", GpuProgramParameters::ACT_LIGHT_ATTENUATION);
							params->setNamedAutoConstant("spotParams", GpuProgramParameters::ACT_SPOTLIGHT_PARAMS);
							params->setNamedAutoConstant("spotDirection", GpuProgramParameters::ACT_LIGHT_DIRECTION_OBJECT_SPACE);
